...
void output_angles(void)
{
  ...
  else if (output_format == OUTPUT__FORMAT_TEXT)          // standard output ...
  {
    Serial.print("#YPR=");
...
void output_calibration(int calibration_sensor)
{
  ...
  else if (calibration_sensor == 1)  // Magnetometer
  {
    ...
    for (int i = 0; i < 3; i++) {
      if (magn[i] < magnetom_min[i]) magnetom_min[i] = magn[i];
      if (magn[i] > magnetom_max[i]) magnetom_max[i] = magn[i];
...
void output_sensors_text(char raw_or_calibrated)
{
  ...
  Serial.print(magn[0]); Serial.print(",");
  Serial.print(magn[1]); Serial.print(",");
  Serial.print(magn[2]); Serial.println();
...
void output_sensors_binary(void)
{
  Serial.write((byte*) accel, 12);
  Serial.write((byte*) magn, 12);
...
void output_sensors(void)
{
  ...
  else if (output_mode == OUTPUT__MODE_SENSORS_BOTH)
  {
    ...
    else if (output_format == OUTPUT__FORMAT_TEXT)    // Too much for the 8Mhz processor!
    {
      output_sensors_text('R');
      compensate_sensor_errors();
      output_sensors_text('C');
...
void output_whatever(void)
{

  if (output_mode == OUTPUT__MODE_CALIBRATE_SENSORS)  // We're in calibration mode
  {
    check_reset_calibration_session();  // Check if this session needs a reset
    if (output_stream_on_flag || output_single_on_flag) output_calibration(curr_calibration_sensor);
  }

  else if (output_mode == OUTPUT__MODE_ANGLES)  // Output angles to serial
  {
    steady_now_steady();   // We may as well move the servos as if flying while monitoring the output angles
    if (output_stream_on_flag || output_single_on_flag) output_angles();
  }

  else  // Output sensor values
  {      
    if (output_stream_on_flag || output_single_on_flag) output_sensors();
  }

  output_single_on_flag = false;
}
